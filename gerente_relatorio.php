...
//$campoano = $_POST["campoano"];
//$campomes = $_POST["campomes"];
...
<div>
			...
			<form name="form1" method="post" action="gerente_relatorio.php">
			<fieldset>
				<h1>Relat&oacute;rio de Empr&eacute;stimo</h1><br><br>
				<!--<h3>M&ecirc;s
				<input type="text" name="campomes" size="8" maxlength="10"/>
				<input type="submit" value="Buscar"></h3>
				<h3>Ano
				<input type="text" name="campoano" size="8" maxlength="10"/>
				<input type="submit" value="Buscar"></h3>-->
			...
			<br>
				...
				<table id="tabelafundo"><form>
					...
					$query = mysql_query ("SELECT * FROM emprestimo WHERE DataEmprestimo between '$dataini' AND '$datafim' AND Situacao = 'Finalizado';");
			...
			&nbsp;&nbsp;&nbsp;Gerar Relat&oacute;rio em:
				<?php $query2 = mysql_query("SELECT SUM(Multa) AS 'Valor' FROM emprestimo WHERE (DataEmprestimo between '$dataini' AND '$datafim') AND Situacao = 'Finalizado';"); 
				
				while($row2 = mysql_fetch_array($query2)){
	
				$valortotal = $row2['Valor'];
				}
				
		
				?>
				<form name="form2" method="post" action="exportar_pdf.php" target = "blink">
			    <input type="hidden" name="datainicio" value="<?php echo $dataini; ?>" />
			    <input type="hidden" name="datafinal" value="<?php echo $datafim; ?>" />
				<input type="hidden" name="valortotal" value="<?php echo $valortotal; ?>" />
			    &nbsp;&nbsp;&nbsp;<input type="image" src="images/pdf.png"></form>
				
				
				<form name="form1" method="post" action="exportar_excel.php" target = "blink">
			    <input type="hidden" name="datainicio" value="<?php echo $dataini; ?>" />
			    <input type="hidden" name="datafinal" value="<?php echo $datafim; ?>" />
			    &nbsp;&nbsp;&nbsp;<input type="image" src="images/excel.png"></form><br>
				<br><br>
