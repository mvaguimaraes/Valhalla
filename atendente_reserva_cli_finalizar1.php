...
echo "<script>alert('Todos os exemplares desse filme ja foram alugados, a reserva nao pode ser concluida!');top.location.href='atendente_index.php';</script>";
}
	...
	mysql_query("UPDATE  `valhalla`.`reservacliente` SET `Situacao`='Finalizada' WHERE `reservacliente`.`CodReservaCliente` = '$id';");

	mysql_query("UPDATE  `valhalla`.`filme` SET `Quantidade`=`Quantidade`-1 WHERE `filme`.`Nome` LIKE '$filme';");
	
	mysql_query($query,$conexao);

	echo "<script>alert('Sua reserva foi finalizada, o emprestimo foi registrado!');top.location.href='atendente_index.php';</script>";
