...
<div>
			...
			<form name="form1" method="post" action="atendente_pesquisar_reserva.php">
			<fieldset>
				<h1>Pesquisar Reserva Atendente</h1><br><br>
			...
			<fieldset>
				...
				<table id="tabelafundo"><form>
					...
					<?php
					//$query = mysql_query ("SELECT * FROM reservaatendente WHERE MONTH(DataRequisitada) = '$campomes' OR YEAR(DataRequisitada)='$campoano'");
					
					/*if ($campomes == NULL && $campoano != NULL){
					$query = mysql_query ("SELECT * FROM reservaatendente WHERE MONTH(DataRequisitada) = '$campomes' OR YEAR(DataRequisitada)='$campoano'");}
								
					else {
					$query = mysql_query ("SELECT * FROM reservaatendente");}*/
					
					$query = mysql_query ("SELECT * FROM reservaatendente WHERE DataRequisitada between '$dataini' AND '$datafim' AND `CodAtendente` LIKE '$usuario' AND `Situacao` = 'Andamento';");
					...
					<tr>
						<td><a href="atendente_visualizar_reserva_at.php?reservaatendente=<?php echo $row['CodReservaAtendente']?>" ><?php echo $row['CodReservaAtendente'];?></a></td>
			...
			<form name="form2" method="post" action="atendente_editar_reserva_at2.php">
			<input type="submit" value="Editar">
			<input type="hidden" name="dataini" value="<?php echo $dataini; ?>" />
			<input type="hidden" name="datafim" value="<?php echo $datafim; ?>" />
			</form>
			<form name="form3" method="post" action="atendente_excluir_reserva_at.php">
			<input type="hidden" name="dataini" value="<?php echo $dataini; ?>" />
			<input type="hidden" name="datafim" value="<?php echo $datafim; ?>" />
